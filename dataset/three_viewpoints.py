...
class ThreeViewsData(Dataset):
    ...
    def read_one_view(self,name,selected_index,width,height):
        ...
        if self.data_cfg['dataset_name'] == "VN_SIGN":
            # Sử dụng video_path từ config nếu có, nếu không dùng default
            video_base_path = self.data_cfg.get('video_path', 'Yolo_dataset/Blur_video')
            path = f'{video_base_path}/{name}'
    ...
    def read_videos(self,center,left,right):
        ...
        # 
        video_base_path = self.data_cfg.get('video_path', 'Yolo_dataset/Blur_video')
        vlen1,c_width,c_height = self.count_frames(os.path.join(video_base_path,center))
        vlen2,l_width,l_height = self.count_frames(os.path.join(video_base_path,left))
        vlen3,r_width,r_height = self.count_frames(os.path.join(video_base_path,right))
    ...
    def __getitem__(self, idx):
        # Thử đọc sample hiện tại, nếu lỗi thì lấy sample tiếp theo
        max_attempts = 10
        for attempt in range(max_attempts):
            try:
                self.transform.randomize_parameters()
                
                current_idx = (idx + attempt) % len(self.train_labels)
                center,left,right,label = self.train_labels.iloc[current_idx].values
                ...
                if attempt < max_attempts - 1:
                    ...
                    # Nếu thử quá nhiều lần vẫn lỗi, raise exception
                    raise e
